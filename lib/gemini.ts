...
export function generateSmartFallbackComment(
  ...
  switch (archId) {
    case "01": // 폼 미쳤다, 무대 찢는 호랑이
      return {
        characterAnalysis: `너의 끼는 동아리방에 갇혀있을 수 없어! 화려한 조명 아래서 가장 빛나는 카리스마 호랑이입니다.`,
        ...
        cheeringMessage: `🐯 화려한 조명 아래서 가장 눈부시게 무대를 찢어놓을 호랑이를 응원해!`,
        generatedByAi: false,
      };

    case "02": // 밤샘 코딩하다 커피 수액 맞는 호랑이
      return {
        characterAnalysis: `오류창과 기싸움하느라 오늘 밤도 순삭! 코드는 돌아가는데 내 멘탈이 안 돌아가네. 끝까지 밤을 새워 완성해내는 몰입의 끝판왕 호랑이입니다.`,
    ...
    case "03": // 안암골 천사 호랑이
      return {
        characterAnalysis: `세상을 더 나은 곳으로 만들려는 너의 마음! 정의감과 따뜻함으로 세상을 구한다! 캠퍼스 전체를 환하게 밝히는 선한 영향력의 소유자입니다.`,
    ...
    case "04": // 팩트로 뼈 때리는 호랑이
      return {
        characterAnalysis: `팩트와 논리로 무장했다! 당신의 스피치에 모두가 집중할 수밖에! (조금만 짧게 말해줘..) 철저한 논리와 달변의 소유자입니다.`,
        ...
        cheeringMessage: `🐯 날카로운 팩트와 논리로 청중을 사로잡을 고려대 최고의 브레인, 파이팅!`,
        generatedByAi: false,
      };

    case "05": // 근손실 걱정하는 호랑이
      return {
        characterAnalysis: `캠퍼스는 너의 트랙! 과잠보다 운동복이 더 잘 어울리는 너, 오늘 혹시 하체 조지러 가는 날? 지치지 않는 활력과 강철 체력의 소유자입니다.`,
    ...
    case "07": // 낭만 줍는 호랑이
      return {
        characterAnalysis: `남들은 그냥 지나치는 풍경도 렌즈를 거치면 작품이 돼! 안암골 낭만은 네가 다 줍고 다니는구나. 캠퍼스의 모든 찰나를 영화로 만드는 감성 크리에이터입니다.`,
    ...
    default:
      return {
        characterAnalysis: `아직 어디에도 속하지 않은 너, 그건 어디든 갈 수 있다는 뜻! 무한한 가능성의 새내기! 고려대 크림슨 과잠이 가장 잘 어울립니다.`,
        clubSynergy: `'${topClub?.name || "고려대 동아리"}'에서 109개 동아리 중 가장 당신다운 첫 발걸음을 떼며 가슴 뛰는 캠퍼스 이야기를 시작해보세요.`,
        campusTips: `중앙광장 본관 앞 호랑이 잔디밭에서 인생샷 남기고, 9월 초 동아리 부스를 투어하며 온갖 동방을 탐험해보세요!`,
        cheeringMessage: `🐯 어흥! 무한한 잠재력으로 캠퍼스를 정복할 갓기 호랑이, 환영해!`,
        generatedByAi: false,
      };
  }
}
